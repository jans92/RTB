...
$getNoticias = $conn->prepare("SELECT * FROM noticias WHERE idCategoria ='2' ");
    $getNoticias->execute();
    ...
    foreach ($noticias as $noticia) {
        ...
        if (isset($_SESSION['esadmin']) && $_SESSION['esadmin']){
            ...
            echo'<div class="card2">';
              ...
              echo' <div class="modificarphp" style="float:right; cursor:pointer; font-size: 20px;font-family:letra2; margin-left: 2em;  margin-right:4em;color:black;" value="'.$noticia['idNoticia'].'"> MODIFICAR</div> ';
                
        echo '<header class="encabezado">';
        echo '<p >
        <span class="fecha">'.$noticia['fechaModificacion'].'</span> on <a class="categoria" >'.$categoria['nombre'].'</a>
        </p>';
        echo ' <form id="formp-'.$noticia['idNoticia'].'" name="formp-'.$noticia['idNoticia'].'">'; 
